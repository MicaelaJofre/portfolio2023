  ...
  return (
    <div className='w-full cursor-pointer' onClick={handleShow}>
      <div className="shadow-md p-2 w-full flex justify-between items-center text-sm font-semibold text-gray-1">
        <h2>{title}</h2>
        <Image className={`cursor-pointer transition-all duration-500 ${show ? 'rotate-180' : ''}`} width={20} height={20} alt="arrow" src={arrow} />
      </div>
      <div className={`overflow-hidden transition-all duration-500 ease-in-out p-2 w-full flex justify-between items-center ${show ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
